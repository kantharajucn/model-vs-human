...
from dataset import ModelVSHumanDataset
from model_initialization_registry import list_models
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    models_initializers = list_models()
    if args.arch in models_initializers:
        model = eval(f"initializers.{args.arch}")(model)
    else:
        print("Initializer doesn't exist for the given model")
        return
    ...
    # Data loading code
    train_csv_file = f"{ROOT_DIR}/input/train.csv"
    val_csv_file = f"{ROOT_DIR}/input/test.csv"
    ...
    train_dataset = ModelVSHumanDataset(train_csv_file, transformation)
    val_dataset = ModelVSHumanDataset(val_csv_file, transformation)
...
def accuracy(label_type, output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        pred = pred.t()
        if label_type == "hard_labels":
            correct = pred.eq(target.view(1, -1).expand_as(pred))
        elif label_type == "soft_labels":
